...
func test_parse_vector3i() -> void:
    var reader = StringReader.new('1 2 3')
    var result = f.vector3().parse(reader)
    ...
    asserts.is_equal_approx_(result.z.value, 3.0)


func test_parse_vector3() -> void:
    var reader = StringReader.new('1.2 -2.3 3.0')
    var result = f.vector3().parse(reader)
...
func test_relative() -> void:
    ...
    asserts.is_equal_approx_(result.z.value, 3.0)
    asserts.is_true(result.z.relative)
...
func test_global() -> void:
    ...
    asserts.is_equal_approx_(result.z.value, 3.0)
    asserts.is_true(result.z.global)
...
func test_wrong_mixed() -> void:
    ...
    reader = StringReader.new('1 ^^2 3')
    result = f.vector3().parse(reader)
    asserts.is_not_null(result.error)
...
func test_correct_mixed() -> void:
    ...
    asserts.is_equal_approx_(result.z.value, 3.0)
    asserts.is_true(result.z.global)
    asserts.is_false(result.z.relative)
